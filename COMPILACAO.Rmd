...
```{r plot likert1, echo=FALSE, message=FALSE, warning=FALSE}
library(tidyverse)
library(ggplot2)
library(RColorBrewer)
library(reshape2)
...
pal<-c(pal[1:(ceiling(numlevels/2)-1)], pal[ceiling(numlevels/2)], 
       pal[ceiling(numlevels/2)], pal[(ceiling(numlevels/2)+1):(numlevels-1)])
tab3<-melt(tab2,id="outcome") #actualizar para pivot_longer
tab3$col<-rep(pal,each=temp.rows)
tab3$value<-tab3$value*100
tab3$outcome<-str_wrap(tab3$outcome, width = 40)
tab3$outcome<-factor(tab3$outcome, levels = tab2$outcome[order(-(tab2[,5]+tab2[,6]+tab2[,7]))])
...
```
Exemplos  
```{r plot likert2, echo=T, message=FALSE, warning=FALSE}
...
```
```{r plot likert3, echo=T, fig.height=9, fig.width=6, message=FALSE, warning=FALSE}
ResultyG_1 <- likert(Personalityyy[c(1:6)], grouping=Personalityyy$CHANGE_Class3)
ResultyGs_1 <- likert(summary = ResultyG_1$results, grouping = ResultyG_1$results[,1])
...
```
```{r stargazer tableshtml, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, results="asis"}
#só para contínuas, estilo latex
...
#isto precisa de um melhor ajuste..
```
```{r summary tables, eval=F}
...
st_css()
#pelos vistos isto tem de ser metido aqui antes, no markdown, apesar de já estar lá em cima..
```
```{r summary tables show, echo=FALSE, message=FALSE, warning=FALSE}
print(DescStat, method = "render") #valid.col=F,
```


##Matrizes de correlação, em plots bonitos
```{r corr matrix, eval=F}
library(Hmisc)
library(corrplot)
library(ggcorrplot)
...
```
```{r run corr matrix, include=FALSE}
library(Hmisc)
library(corrplot)
library(ggcorrplot)
```
...
```{r model DCM prep fastdummies, eval=F}
library(fastDummies)
...
```{r stplanr agregate, eval=T, echo=T, fig.height=4, fig.width=5, message=FALSE, warning=FALSE}
